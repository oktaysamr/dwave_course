...
def get_bqm(S):
    ...
    variable = ['box_17', 'box_21', 'box_19']

    for i in range(len(S)):
        bqm.add_variable(variable[i], S[i])

    bqm.add_linear_equality_constraint([(variable[i], 1) for i in range(len(S))], lagrange_multiplier=20, constant= -2)
